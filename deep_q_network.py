...
def createNetwork():
    ...
    W_fc2 = weight_variable([512, N_ACTIONS])
...
def trainNetwork(s, readout, h_fc1, sess):
    ...
    a = tf.compat.v1.placeholder("float", [None, N_ACTIONS])
    ...
    while "flappy bird" != "angry bird":
        ...
        if t % FRAME_PER_ACTION == 0:
            if random.random() <= epsilon:
                ...
                a_t[random.randrange(N_ACTIONS)] = 1
            else:
        ...
        print("GRAYSCALE shape -> ", x_t1.shape)
        ret, x_t1 = cv2.threshold(x_t1, 1, 255, cv2.THRESH_BINARY)
        # print("Thresholded shape -> ", x_t1.shape)
        x_t1 = np.reshape(x_t1, (80, 80, 1))
        print("Reshaped shape -> ", x_t1.shape)
        cv2.imshow("Processed Image", x_t1)
